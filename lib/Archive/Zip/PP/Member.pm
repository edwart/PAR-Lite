...
BEGIN {
    $VERSION = '1.30';
    @ISA     = qw( Archive::Zip::PP );
}

use Archive::Zip::PP qw(
  :CONSTANTS
  :MISC_CONSTANTS
  :ERROR_CODES
  :PKZIP_CONSTANTS
...
use Time::Local ();
use Compress::Zlib::Perl qw( Z_OK Z_STREAM_END MAX_WBITS );
use File::Path;
use File::Basename;

use constant ZIPFILEMEMBERCLASS   => 'Archive::Zip::PP::ZipFileMember';
use constant NEWFILEMEMBERCLASS   => 'Archive::Zip::PP::NewFileMember';
use constant STRINGMEMBERCLASS    => 'Archive::Zip::PP::StringMember';
use constant DIRECTORYMEMBERCLASS => 'Archive::Zip::PP::DirectoryMember';
...
sub _writeSymbolicLink {
    my $self = shift;
    my $name = shift;
    my $chunkSize = $Archive::Zip::PP::ChunkSize;
...
sub readChunk {
    ...
    $chunkSize = $Archive::Zip::PP::ChunkSize if not defined($chunkSize);
...
sub rewindData {
    ...
    {
        ( $self->{'deflater'}, $status ) = Compress::Zlib::Perl::Deflate->new(
            '-Level'      => $self->desiredCompressionLevel(),
            '-WindowBits' => -MAX_WBITS(),                     # necessary magic
            '-Bufsize'    => $Archive::Zip::PP::ChunkSize,
            @_
    ...
    {
        ( $self->{'inflater'}, $status ) = Compress::Zlib::Perl::Inflate->new(
            '-WindowBits' => -MAX_WBITS(),               # necessary magic
            '-Bufsize'    => $Archive::Zip::PP::ChunkSize,
            @_
        );    # pass additional options
